...
void make_level_set3(const std::vector<Vec3ui> &tri, const std::vector<Vec3f> &x,
                     ...
                     Array3f &phi, Array3c &phi_sign, const int exact_band)
{
   phi.resize(ni, nj, nk);
   phi.assign((ni+nj+nk)*dx); // upper bound on distance
   phi_sign.resize(ni, nj, nk);
   phi_sign.assign(0); // 0 is inside, 1 is outside
   Array3i closest_tri(ni, nj, nk, -1);
   Array3i intersection_count_i(ni+1, nj, nk, 0); // intersection_count_i(i,j,k) is # of tri intersections in (i-1,i]x{j}x{k}
   Array3i intersection_count_j(ni, nj+1, nk, 0); // intersection_count_j(i,j,k) is # of tri intersections in {i}x(j-1,j]x{k}
   Array3i intersection_count_k(ni, nj, nk+1, 0); // intersection_count_k(i,j,k) is # of tri intersections in {i}x{j}x(k-1,k]
   ...
   for(unsigned int t=0; t<tri.size(); ++t){
     ...
     // coordinates in grid to high precision
      ...
      i0=clamp((int)std::ceil(min(fip,fiq,fir)), 0, ni-1);
      i1=clamp((int)std::floor(max(fip,fiq,fir)), 0, ni-1);
      ...
      for(int j=j0; j<=j1; ++j) for(int k=k0; k<=k1; ++k){
         ...
         if(point_in_triangle_2d(j, k, fjp, fkp, fjq, fkq, fjr, fkr, a, b, c)){
            ...
            if(i_interval<0) ++intersection_count_i(0, j, k); // we enlarge the first interval to include everything to the -x direction
            else if(i_interval<ni) ++intersection_count_i(i_interval,j,k);
            else ++intersection_count_i(ni,j,k); // we enlarge the last interval to include everything to the +x direction
         }
      }
      for(int i=i0; i<=i1; ++i) for(int k=k0; k<=k1; ++k){
         double a, b, c;
         if(point_in_triangle_2d(i, k, fip, fkp, fiq, fkq, fir, fkr, a, b, c)){
            double fj=a*fjp+b*fjq+c*fjr; // intersection j coordinate
            int j_interval=int(std::ceil(fj)); // intersection is in (j_interval-1,j_interval]
            if(j_interval<0) ++intersection_count_j(i, 0, k); // we enlarge the first interval to include everything to the -y direction
            else if(j_interval<nj) ++intersection_count_j(i,j_interval,k);
            else ++intersection_count_j(i,nj,k); // we enlarge the last interval to include everything to the +y direction
         }
      }
      for(int i=i0; i<=i1; ++i) for(int j=j0; j<=j1; ++j){
         double a, b, c;
         if(point_in_triangle_2d(i, j, fip, fjp, fiq, fjq, fir, fjr, a, b, c)){
            double fk=a*fkp+b*fkq+c*fkr; // intersection k coordinate
            int k_interval=int(std::ceil(fk)); // intersection is in (k_interval-1,k_interval]
            if(k_interval<0) ++intersection_count_k(i, j, 0); // we enlarge the first interval to include everything to the -z direction
            else if(k_interval<nk) ++intersection_count_k(i,j,k_interval);
            else ++intersection_count_k(i,j,nk); // we enlarge the last interval to include everything to the +z direction
         }
      }
   }
   ...
   // carving...
   for(int j=0; j<nj; ++j) for(int k=0; k<nk; ++k){
   //x+
      for(int i=0; i<ni; ++i){
         if (intersection_count_i(i,j,k)>0) break;
         else phi_sign(i,j,k)=1;
      }
   //x-
      for(int i=ni; i>0; --i){
         if (intersection_count_i(i,j,k)>0) break;
         else phi_sign(i-1,j,k)=1;
      }
   }
   for(int i=0; i<ni; ++i) for(int j=0; j<nj; ++j){
   //z+
      for(int k=0; k<nk; ++k){
         if (intersection_count_k(i,j,k)>0) break;
         else phi_sign(i,j,k)=1;
      }
   //z-
      for(int k=nk; k>0; --k){
         if (intersection_count_k(i,j,k)>0) break;
         else phi_sign(i,j,k-1)=1;
      }
   }
   for(int i=0; i<ni; ++i) for(int k=0; k<nk; ++k){
   //y+
   // no y+ because y+ is the bottom-to-up view
   //y-
      for(int j=nj; j>0; --j){
         if (intersection_count_j(i,j,k)>0) break;
         else phi_sign(i,j-1,k)=1;
      }
   }

   for(int i=0; i<ni; ++i) for(int j=0; j<nj; ++j) for(int k=0; k<nk; ++k){
      if (phi_sign(i,j,k)>0) phi(i,j,k)=-phi(i,j,k);
   }
}
