...
int main(int argc, char* argv[]) {
  ...
  if(argc != 4) {
    std::cout << "IntersectionXYZpn - A utility for sampling points with normals on triangle meshes.\n";
    std::cout << "\nThe sampled points are intersection points between the mesh and the edges of a uniform grid.\n";
    std::cout << "The output filename will match that of the input, with the OBJ suffix replaced with .intersectionpn.\n\n";

    std::cout << "Usage: IntersectionXYZpn <filename> <size> <padding>\n\n";
    std::cout << "Where:\n";
  ...
  outname = filename.substr(0, filename.size()-4) + std::string(".intersectionpn");
  //std::cout << "Writing results to: " << outname << "\n";
  
  std::ofstream outfile( outname.c_str());
  outfile << "#intersectionpn 1\n";
  ...
  for(int i=0; i<phi_X_N; ++i) for(int j=0; j<6; ++j) {
    *floatv = phi_X(i,j);
    outfile << charv[0] << charv[1] << charv[2] << charv[3];
  }
  for(int i=0; i<phi_Y_N; ++i) for(int j=0; j<6; ++j) {
    *floatv = phi_Y(i,j);
    outfile << charv[0] << charv[1] << charv[2] << charv[3];
  }
  for(int i=0; i<phi_Z_N; ++i) for(int j=0; j<6; ++j) {
    *floatv = phi_Z(i,j);
    outfile << charv[0] << charv[1] << charv[2] << charv[3];
  }
