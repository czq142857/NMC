...
class ABC_nmc_hdf5(torch.utils.data.Dataset):
    def __init__(self, data_dir, train, input_type, out_bool, out_float, input_only=False):
        ...
        temp_hdf5_names = []
        temp_hdf5_gridsizes = []
        if self.train and self.input_type=="sdf" and not self.out_float:
            for name in self.hdf5_names:
                hdf5_file = h5py.File(self.data_dir+"/"+name, 'r')
                for grid_size in [32,64]:
                    gt_output_bool_ = hdf5_file[str(grid_size)+"_int"][:]
                    gt_output_bool_mask_ = (gt_output_bool_>=0)
                    gt_output_bool_mask_[:,:,:,0] = 0
                    if np.sum(gt_output_bool_mask_)>0:
                        temp_hdf5_names.append(name)
                        temp_hdf5_gridsizes.append(grid_size)
        elif self.train and not self.out_bool:
            for name in self.hdf5_names:
                hdf5_file = h5py.File(self.data_dir+"/"+name, 'r')
                for grid_size in [32,64]:
                    gt_case_id = hdf5_file[str(grid_size)+"_case_id"][:]
                    if np.sum(gt_case_id>0)>0:
                        temp_hdf5_names.append(name)
                        temp_hdf5_gridsizes.append(grid_size)
        elif self.train:
            for name in self.hdf5_names:
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            if self.input_type=="sdf" and not self.out_float:
                gt_case_id,gt_output_bool_,gt_output_float_,gt_input_ = read_and_augment_data(hdf5_dir,grid_size,self.input_type,self.out_bool,self.out_float,aug_permutation=True,aug_reversal=True,aug_inversion=True)
            else:
                gt_case_id,gt_output_bool_,gt_output_float_,gt_input_ = read_and_augment_data(hdf5_dir,grid_size,self.input_type,self.out_bool,self.out_float,aug_permutation=True,aug_reversal=True,aug_inversion=False)
        else:
            if self.input_only:
                gt_case_id,gt_output_bool_,gt_output_float_,gt_input_ = read_data_input_only(hdf5_dir,grid_size,self.input_type,self.out_bool,self.out_float)
            else:
                gt_case_id,gt_output_bool_,gt_output_float_,gt_input_ = read_data(hdf5_dir,grid_size,self.input_type,self.out_bool,self.out_float)

        if self.out_bool:
            gt_output_bool_ = np.transpose(gt_output_bool_, [3,0,1,2]).astype(np.float32)
            gt_output_bool_mask_ = (gt_output_bool_>=0).astype(np.float32)
            gt_output_bool_mask_[0,:,:,:] = 0
        ...
        if self.train:
            if not self.out_float:
                valid_flag = np.sum(gt_output_bool_mask_, axis=0)>0
            elif not self.out_bool:
                valid_flag = gt_case_id>0
            else:
                valid_flag = (np.sum(gt_output_bool_mask_, axis=0)>0) | (gt_case_id>0)
