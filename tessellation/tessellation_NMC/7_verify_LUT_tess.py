...
LUT_file = np.load("LUT_tess.npz")
LUT_tess = LUT_file["LUT_tess"]
...
ti = 0
while True:
	if LUT_tess[this_idx,ti,0]<0: break
	
	v1i = LUT_tess[this_idx,ti,0]
	v2i = LUT_tess[this_idx,ti,1]
	v3i = LUT_tess[this_idx,ti,2]
	
	if [v1i,v2i] not in load_connections and [v2i,v1i] not in load_connections:
		load_connections.append([v1i,v2i])
	if [v1i,v3i] not in load_connections and [v3i,v1i] not in load_connections:
		load_connections.append([v1i,v3i])
	if [v3i,v2i] not in load_connections and [v2i,v3i] not in load_connections:
		load_connections.append([v3i,v2i])
...
temp_inversed = np.sum(config[:8])>4
if temp_inversed: config[:14] = 1-config[:14]
for fi in range(6):
	...
	if p1v == 0 and p2v == 0 and p3v == 1 and p4v == 1:
		prepared_connections.append( [find_edge_point_from_cube_points[p3i,p2i],find_face_point_from_cube_point_and_face[p3i,fi]] )
		prepared_connections.append( [find_edge_point_from_cube_points[p4i,p1i],find_face_point_from_cube_point_and_face[p4i,fi]] )
		prepared_connections.append( [find_face_point_from_cube_point_and_face[p3i,fi],find_face_point_from_cube_point_and_face[p4i,fi]] )
	...
	if p1v == 0 and p2v == 1 and p3v == 1 and p4v == 0:
		prepared_connections.append( [find_edge_point_from_cube_points[p2i,p1i],find_face_point_from_cube_point_and_face[p2i,fi]] )
		prepared_connections.append( [find_edge_point_from_cube_points[p3i,p4i],find_face_point_from_cube_point_and_face[p3i,fi]] )
		prepared_connections.append( [find_face_point_from_cube_point_and_face[p2i,fi],find_face_point_from_cube_point_and_face[p3i,fi]] )
	...
	if p1v == 1 and p2v == 0 and p3v == 0 and p4v == 1:
		prepared_connections.append( [find_edge_point_from_cube_points[p1i,p2i],find_face_point_from_cube_point_and_face[p1i,fi]] )
		prepared_connections.append( [find_edge_point_from_cube_points[p4i,p3i],find_face_point_from_cube_point_and_face[p4i,fi]] )
		prepared_connections.append( [find_face_point_from_cube_point_and_face[p1i,fi],find_face_point_from_cube_point_and_face[p4i,fi]] )
	...
	if p1v == 1 and p2v == 1 and p3v == 0 and p4v == 0:
		prepared_connections.append( [find_edge_point_from_cube_points[p1i,p4i],find_face_point_from_cube_point_and_face[p1i,fi]] )
		prepared_connections.append( [find_edge_point_from_cube_points[p2i,p3i],find_face_point_from_cube_point_and_face[p2i,fi]] )
		prepared_connections.append( [find_face_point_from_cube_point_and_face[p1i,fi],find_face_point_from_cube_point_and_face[p2i,fi]] )
...
if len(load_connections)>0:
	print("\n\nConnections loaded")
	prepared_connections = load_connections
	#check if the used points are valid according to all_points_visibility
	for i in range(len(prepared_connections)):
		for j in range(2):
			if all_points_visibility[prepared_connections[i][j]]==0:
				print("ERROR: invalid points!", prepared_connections[i][j])
	...
	for j in range(things_to_draw_len):
		...
		if i<all_points_len:
			...
			if i<frame_connections_len: #base frame
				...
				c = (all_points_transformed[p1i,2] + all_points_transformed[p2i,2])*128+160
				c = int(c)
				if c<0: c = 0
				if c>240: c = 240
				color = (c,c,c)
			...
			p3 = (int(all_points_transformed[p3i,0]),int(all_points_transformed[p3i,1]))
			c = (all_points_transformed[p1i,2] + all_points_transformed[p2i,2] + all_points_transformed[p3i,2])*85.333+160
			c = int(c)
			if c<40: c = 40
			if c>240: c = 240
			color = (c,c-40,c-40)
